 ...
 *   --config <path>    path to trunknux.config.mjs for glob overrides
 */
...
function _render(requirements, graph, backlogIndex, existingNotes, stats) {
  ...
  lines.push('<!-- trunknux:rtm generated -->');
  lines.push(`<!-- Generated: ${date} by trunknux rtm. Do NOT edit the table rows directly. -->`);
  ...
  lines.push('');
  lines.push('> **Marker convention:** Each row is wrapped in `<!-- trunknux:row R-XX begin/end -->` markers.');
  ...
  for (const req of requirements) {
    ...
    lines.push(`<!-- trunknux:row ${req.id} begin -->`);
    lines.push(
      `| ${req.id} | ${req.title} | ${req.status} | ${sprintCell} | ${codeCell} | ${testCell} | ${backlogCell} | ${notes} |`,
    );
    lines.push(`<!-- trunknux:row ${req.id} end -->`);
  }

  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('*This file is auto-generated by `trunknux rtm`. Re-run to update evidence columns.*');
...
function _extractNotes(content) {
  ...
  const rowRe = /<!-- trunknux:row (R-\d{2,4}[A-Z]?) begin -->\n\|(.+)\|\n<!-- trunknux:row \1 end -->/g;
  let m;
