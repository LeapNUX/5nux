...
An honest breakdown of what BranchNuX costs to run.

---

## BranchNuX CLI: free

BranchNuX is Apache 2.0 open-source software. The CLI has:
...
The v0.1 core (`branchnux report`, `validate`, `init`, `demo`) has **zero API cost**. It's deterministic — markdown in, HTML + XLSX out.
...
| What | Recommendation |
|---|---|
| **Subscription tier** | **Claude Max 5x** (~$100/mo) — enough headroom for serious BranchNuX usage; upgrade to 20x only if you regularly burn through 5x in under 1 hour |
...
| Opus + 5–8 Sonnet subagents (full multi-agent) | **~1–2 hours** ← what BranchNuX bootstrap looked like |
...
3. **Switch to pay-per-use Anthropic API** — no rate limit, dollar-capped via `--max-spend`. Best for big batch runs (e.g., `branchnux batch-plan` over 30 pages) where you want predictability. Estimate: $10–$40 for a full 30-page site pass.
...
gstack (https://github.com/garrytan/gstack) is OSS with no cost. There is no commercial gstack license required to use BranchNuX.
...
- **branchnux-mcp** (planned v0.3) — local MCP server. No cost.
...
| BranchNuX v0.1 (no AI) | 1–2 hours | $0 |
| BranchNuX v0.2 (AI agents) | 15–30 min | $0.30 – $0.50 API |
