...
At v0.1, the RTM is a hand-maintained `requirements/TRACEABILITY.md`. At v0.2, `branchnux rtm` generates it automatically from the three tracks.
...
| **RTM** | Requirements Traceability Matrix. Maps R-XX → sprint → code → TC → result. Hand-maintained at v0.1; generated by `branchnux rtm` at v0.2. |
...
| **OSCAL** | Open Security Controls Assessment Language. NIST machine-readable format for control assessments. BranchNuX v0.2 emits OSCAL JSON alongside markdown SCA output, satisfying FedRAMP RFC-0024 (mandatory September 2026). |
