 ...
 *
 * Implements `branchnux codify <slug>`.
 *
 ...
 *
 * Usage:
 *   branchnux codify <slug> [--folder <path>] [--model <model>]
...
export async function runCodify(slug, opts = {}) {
  ...
  if (!resolvedFolder || !fs.existsSync(resolvedFolder)) {
    printError(json,
      ...
      '  Suggested workflow:\n' +
      `    1. branchnux discover <url>    # generate scenarios\n` +
      `    2. branchnux plan ${slug}      # generate test-plan.md\n` +
      `    3. branchnux codify ${slug}    # generate spec.ts (this command)\n\n` +
  ...
  if (!fs.existsSync(templatePath)) {
    printError(json,
      ...
      '  Make sure you are running branchnux from the project root and\n' +
  ...
  if (!json) {
    console.log('');
    console.log('  branchnux codify — v0.2 ALPHA');
    console.log('  ─────────────────────────────────────────────────────────');
  ...
  if (json) {
    ...
    console.log(`    4. Then: branchnux report ${slug}`);
    console.log('');
  }
}
...
function handleApiError(err, json, slug) {
  ...
  if (status === 429) {
    ...
    printError(json,
      ...
      '  Options:\n' +
      `    - Wait and re-run: branchnux codify ${slug}\n` +
