 ...
 *
 * Implements `branchnux visual` — visual regression testing.
 *
 * Sub-commands:
 *   branchnux visual baseline <slug>
 *     Capture full-page screenshots for all TCs in <slug>/ as baseline images.
 *     Stored at: <folder>/visual-baseline/<TC-ID>.png
 *
 *   branchnux visual compare <slug>
 ...
 *
 * Configuration (branchnux.config.mjs):
 *   export default {
 *     visual: {
...
// Default visual configuration — overridable via branchnux.config.mjs
...
export async function runVisualBaseline(slug, opts = {}) {
  ...
  if (!json) {
    ...
    console.log('');
    console.log('  Next: run `branchnux visual compare ' + slug + '` to diff against these baselines.');
    console.log('');
  }
...
export async function runVisualCompare(slug, opts = {}) {
  ...
  if (!fs.existsSync(baselineDir)) {
    const msg =
      ...
      `  Run \`branchnux visual baseline ${slug}\` first to capture baselines.`;
    printError(json, msg);
...
async function requirePlaywright(json) {
  ...
  printError(
    ...
    '@playwright/test is not installed.\n\n' +
      ...
      '  Then re-run: branchnux visual baseline <slug>',
  );
...
/**
 * Load visual config from branchnux.config.mjs if it exists.
...
async function loadVisualConfig(outDir) {
  const configPath = path.resolve(outDir, '..', 'branchnux.config.mjs');
  const altConfigPath = path.resolve(process.cwd(), 'branchnux.config.mjs');
