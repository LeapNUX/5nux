...
This page walks through how the BranchNuX model works in practice, then ends with a reference glossary. Read the narrative once; refer back to the glossary as needed.
...
BranchNuX gives you one model that handles all three of these: the requirement, the test, and the proof that the test was executed against the right thing.
...
**The marker convention** keeps the RTM editable by humans without being overwritten by `branchnux rtm` on regeneration. Wrap any hand-authored block in:
...
BranchNuX enforces a fixed taxonomy. Each status has a specific meaning and implies a specific next move:
...
BranchNuX uses a hash-chained UAT log. Every entry in `uat-log.jsonl` contains the HMAC-SHA256 signature of its own payload and the hash of the previous entry. The chain is signed with a shared secret (`UAT_SECRET` in your environment). The result is a ledger where any modification — to any entry, in any position — breaks the chain at that point and is immediately detectable.
...
5. **Sign with a different secret** — all signatures fail `branchnux sign verify` because they do not match `UAT_SECRET`

When the compliance team needs a sign-off artifact for an auditor, the workflow is:

```
branchnux sign verify testing-log/2026-04-26_login/uat-log.jsonl
```
...
The auditor gets a printed ledger PDF. `branchnux sign verify` confirms chain integrity in under a second. The sign-off is either valid or it is not — there is no ambiguity and no way to retroactively alter a past attestation without detection.
...
| **BR-XX** | Business requirement ID. Sits above R-XX — defines the business outcome; R-XX defines the functional implementation. RTM cross-link via `branchnux br`. |
| **RTM** | Requirements Traceability Matrix. Maps R-XX → sprint → code → TC → result. Generated by `branchnux rtm` from the three tracks. |
| **Marker convention** | `<!-- branchnux:row R-XX begin/end -->` — wraps human-authored annotations inside a generated RTM file. Generator preserves content inside markers on regeneration. |
...
| **SCA** | Security Control Assessment. Per-surface document mapping each security control to implementation evidence. Eight sections; generated from test results by `branchnux sca <surface>`. `[VERIFY]` markers flag LLM-generated cells that require human review before submission. |
| **OSCAL** | Open Security Controls Assessment Language. NIST 1.1.2 machine-readable format for control assessments. `branchnux sca-oscal` emits OSCAL JSON alongside the markdown SCA, satisfying FedRAMP RFC-0024 (mandatory September 2026). |
