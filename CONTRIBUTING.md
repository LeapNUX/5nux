...
BranchNuX uses DCO instead of a CLA. This means you attest that your contribution is your own work (or that you have the right to submit it) by signing off every commit:
...
```bash
node bin/branchnux.mjs doctor
node bin/branchnux.mjs demo
```
...
automatically with `npm install branchnux` to keep the base install lightweight.
...
# Smoke-test discover
node bin/branchnux.mjs discover https://example.com --dry-run  # no API call
node bin/branchnux.mjs discover https://example.com            # real call

# Run without the SDK installed → graceful error (no crash)
# npm uninstall @anthropic-ai/sdk
# node bin/branchnux.mjs discover https://example.com
