 ...
 *
 * Implements `branchnux plan <slug>`.
 *
 * v0.2 ALPHA — wired to Claude API (claude-sonnet-4-6 by default).
 *
 * Reads scenarios.md (produced by `branchnux discover`), sends it to the
 * Anthropic Messages API with a structured prompt, and writes a test-plan.md
 * conforming to the BranchNuX schema (YAML frontmatter + TC table format).
 * Every LLM-generated cell is tagged [VERIFY].
 *
 * Usage:
 *   branchnux plan <slug> [--url <url>] [--industry <industry>]
 ...
 *   web applications. Your output must conform exactly to the BranchNuX
 ...
 *
 *   TASK: Convert the scenarios above into a BranchNuX test-plan.md.
 *
 *   OUTPUT REQUIREMENTS:
 *
...
export async function runPlan(slug, opts = {}) {
  ...
  if (!json) {
    console.log('');
    console.log('  branchnux plan — v0.2 ALPHA');
    console.log('  ─────────────────────────────────────────────────────────');
  ...
  if (!scenariosFile) {
    printError(json, slug,
      ...
      `    branchnux discover <url>   # creates scenarios.md for "${slug}"\n\n` +
  ...
  if (json) {
    ...
    console.log(`    2. Run: branchnux codify ${slug}`);
    console.log('');
  }
}
...
function handleApiError(err, json, slug) {
  ...
  if (status === 429) {
    ...
    printError(json, slug,
      ...
      '  Options:\n' +
      '    - Wait and re-run: branchnux plan ' + slug + '\n' +
