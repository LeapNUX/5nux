 ...
 *
 * Implements per-environment test passes for BranchNuX.
 *
 * Sub-commands:
 *   branchnux run <slug> --env staging|prod|local|qa|ci|dev|<custom>
 ...
 *
 *   branchnux compare <slug> <env-a> <env-b>
...
export async function runEnvRun(slug, opts = {}) {
  ...
  if (!json) {
    console.log('');
    console.log(C.bold(`branchnux run — ${slug} [${env.toUpperCase()}]`));
...
export async function runEnvCompare(slug, envA, envB, opts = {}) {
  ...
  if (!folderA) {
    ...
    if (!json) {
      ...
      console.error(C.dim(`    Run: branchnux run ${slug} --env ${envA}`));
    }
    process.exit(2);
  }
  if (!folderB) {
    ...
    if (!json) {
      console.error(C.red(`  ✗ ${msg}`));
      console.error(C.dim(`    Run: branchnux run ${slug} --env ${envB}`));
    }
    process.exit(2);
  }
