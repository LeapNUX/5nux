...
// When TRUN-CLI-0N burns the 5/60s login budget (e.g. 6 wrong attempts),
 ...
 *   // ... test logic ...
 *   await captureEvidence(page, 'TRUN-CLI-05');  // ← before ctx.close()
 *   await ctx.close();
 */
...
test.afterEach(async ({ page }, testInfo) => {
  const match = testInfo.title.match(/^(TRUN-CLI-\d+)/);
  if (!match) return;
  const tcId = match[1];
...
test.describe('TRUN-CLI — full test suite', () => {
  ...
  test('TRUN-CLI-01 — Happy path: [main success scenario]', async ({ page }) => {
    if (!mainTester) {
      record('TRUN-CLI-01', 'BLOCKED-NO-USER', 'main tester not seeded');
      test.skip();
      return;
    }
    ...
    try {
      ...
      record('TRUN-CLI-01', 'PASS', 'Reached /dashboard');
    } catch {
      record('TRUN-CLI-01', 'FAIL', `Did not reach /dashboard. Final URL: ${page.url()}`);
  ...
  test('TRUN-CLI-02 — [Error scenario]', async ({ page }) => {
    await gotoPage(page);
  ...
  test('TRUN-CLI-03 — [Validation: empty required field]', async ({ page }) => {
    await gotoPage(page);
  ...
  test('TRUN-CLI-04 — Tab order: logical keyboard navigation', async ({ page }) => {
    await gotoPage(page);
    ...
    if (foundField1 && foundField2 && foundSubmit) {
      record('TRUN-CLI-04', 'PASS', 'Logical tab order confirmed');
    } else {
      record(
        'TRUN-CLI-04',
        'FAIL',
  ...
  test('TRUN-CLI-05 — Mobile viewport (393×852) no horizontal overflow', async ({
    browser,
  }) => {
    ...
    if (!hasOverflow) {
      record('TRUN-CLI-05', 'PASS', 'No horizontal overflow at 393px');
    } else {
      record('TRUN-CLI-05', 'FAIL', 'Horizontal overflow detected at 393px');
    }
  ...
  // ─── TRUN-CLI-06 (MFA TOTP happy path — skip if no TOTP) ───────────
  // Demonstrates the waitForNextTotpWindow helper for sequential TOTP tests.

  test('TRUN-CLI-06 — MFA TOTP happy path → /dashboard', async ({ page }) => {
    ...
    try {
      await page.waitForURL(/\/dashboard/, { timeout: 8000 });
      record('TRUN-CLI-06', 'PASS', `TOTP code=${code} accepted`);
    } catch {
      record('TRUN-CLI-06', 'FAIL', `Did not reach /dashboard. URL: ${page.url()}`);
  ...
  // @rate-limit-test
  test(
    'TRUN-CLI-0N — Rate limit: N+1 attempts within window returns 429',
    async ({ request }) => {
      ...
      if (has429) {
        record('TRUN-CLI-0N', 'PASS', `responses=[${responses.join(',')}] — 429 hit`);
      } else {
        record(
          'TRUN-CLI-0N',
          'FAIL',
