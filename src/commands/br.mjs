 ...
 * src/commands/br.mjs
 *
 * Implements `testing-hub br` — Business Requirements (BR-XX) management.
 *
 * Sub-commands:
...
export async function runBrInit(id, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    const header = [
      ...
      '',
      '<!-- testing-hub: BUSINESS_REQUIREMENTS v1 -->',
      '<!-- Each ## BR-XX section is managed by `testing-hub br`. -->',
      '',
    ].join('\n');
  ...
  if (!json) {
    ...
    console.log(`  testing-hub br link ${id} R-01,R-02`);
  }
}
...
export async function runBrLink(brId, rIds, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    const err = new Error(
      `${filePath} not found. Run \`testing-hub br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
  ...
  if (!hasBrSection(content, brId)) {
    const err = new Error(
      `${brId} section not found in ${filePath}. Run \`testing-hub br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
...
export async function runBrRtm(opts = {}) {
  ...
  if (!fs.existsSync(brFile)) {
    if (!json) {
      ...
      console.log('  Run: testing-hub br init BR-01');
    }
  ...
  const rtmContent = [
    ...
    '',
    '<!-- testing-hub: UAT_TRACEABILITY v1 — generated by `testing-hub br rtm` -->',
    '<!-- Do not hand-edit the table; re-run `testing-hub br rtm` to refresh. -->',
    '',
...
function buildBrSection(id) {
  return [
    ...
    '### Linked R-IDs',
    '',
    '_(none yet — run `testing-hub br link ' + id + ' R-01,R-02`)_',
    '',
    '### Linked TC-IDs',
    '',
    '_(populated automatically via RTM — run `testing-hub br rtm`)_',
    '',
