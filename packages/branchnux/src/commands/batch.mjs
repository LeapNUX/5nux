 ...
 *
 * Implements `branchnux batch-plan --pages "login,register,dashboard,..."`.
 *
 ...
 *
 * Usage:
 *   branchnux batch-plan --pages "login,register,dashboard"
 *   branchnux batch-plan --pages "login=https://example.com/login,register=https://example.com/register"
 *   branchnux batch-plan --pages "login,register" --stages "discover,plan" --max-spend 5
 *   branchnux batch-plan --pages "login" --dry-run
 *
 * Exit codes:
...
export async function runBatchPlan(opts = {}) {
  ...
  if (dryRun) {
    if (json) {
      ...
      console.log('');
      console.log('  branchnux batch-plan — DRY RUN');
      console.log('  ─────────────────────────────────────────────────────────');
  ...
  if (!json) {
    console.log('');
    console.log('  branchnux batch-plan — v0.2 ALPHA');
    console.log('  ─────────────────────────────────────────────────────────');
...
function writeBatchSummary(results, enabledStages, totalCost, outDir) {
  ...
  const content = [
    ...
    '',
    '_Generated by branchnux batch-plan v0.2_',
  ].join('\n');
