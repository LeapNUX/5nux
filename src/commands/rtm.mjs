 ...
 *   --config <path>    path to testing-hub.config.mjs for glob overrides
 */
...
function _render(requirements, graph, backlogIndex, existingNotes, stats) {
  ...
  lines.push('<!-- testing-hub:rtm generated -->');
  lines.push(`<!-- Generated: ${date} by testing-hub rtm. Do NOT edit the table rows directly. -->`);
  ...
  lines.push('');
  lines.push('> **Marker convention:** Each row is wrapped in `<!-- testing-hub:row R-XX begin/end -->` markers.');
  ...
  for (const req of requirements) {
    ...
    lines.push(`<!-- testing-hub:row ${req.id} begin -->`);
    lines.push(
      `| ${req.id} | ${req.title} | ${req.status} | ${sprintCell} | ${codeCell} | ${testCell} | ${backlogCell} | ${notes} |`,
    );
    lines.push(`<!-- testing-hub:row ${req.id} end -->`);
  }

  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('*This file is auto-generated by `testing-hub rtm`. Re-run to update evidence columns.*');
...
function _extractNotes(content) {
  ...
  const rowRe = /<!-- testing-hub:row (R-\d{2,4}[A-Z]?) begin -->\n\|(.+)\|\n<!-- testing-hub:row \1 end -->/g;
  let m;
