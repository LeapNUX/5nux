 ...
 *
 * Implements `branchnux validate <folder>`.
 *
...
// as WARNINGS instead of ERRORS so `branchnux init` → `branchnux validate`
...
export async function runValidate(folder, opts = {}) {
  ...
  if (!fs.existsSync(specFile)) {
    findings.warnings.push({
      file: 'spec.ts',
      rule: 'folder.spec',
      message: 'spec.ts not found — run `branchnux init` to scaffold, or create manually',
    });
  }
