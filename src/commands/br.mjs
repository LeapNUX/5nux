 ...
 * src/commands/br.mjs
 *
 * Implements `trunknux br` — Business Requirements (BR-XX) management.
 *
 * Sub-commands:
...
export async function runBrInit(id, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    const header = [
      ...
      '',
      '<!-- trunknux: BUSINESS_REQUIREMENTS v1 -->',
      '<!-- Each ## BR-XX section is managed by `trunknux br`. -->',
      '',
    ].join('\n');
  ...
  if (!json) {
    ...
    console.log(`  trunknux br link ${id} R-01,R-02`);
  }
}
...
export async function runBrLink(brId, rIds, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    const err = new Error(
      `${filePath} not found. Run \`trunknux br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
  ...
  if (!hasBrSection(content, brId)) {
    const err = new Error(
      `${brId} section not found in ${filePath}. Run \`trunknux br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
...
export async function runBrRtm(opts = {}) {
  ...
  if (!fs.existsSync(brFile)) {
    if (!json) {
      ...
      console.log('  Run: trunknux br init BR-01');
    }
  ...
  const rtmContent = [
    ...
    '',
    '<!-- trunknux: UAT_TRACEABILITY v2 — generated by `trunknux br rtm` -->',
    '<!-- Do not hand-edit the table; re-run `trunknux br rtm` to refresh. -->',
    '',
 ...
 * Used by `trunknux sign` to determine if attestation completes the BR.
 *
 * @param {string} brId             - e.g. "BR-01"
...
function buildBrSection(id) {
  return [
    ...
    '### Linked R-IDs',
    '',
    '_(none yet — run `trunknux br link ' + id + ' R-01,R-02`)_',
    '',
    '### Linked TC-IDs',
    '',
