# Sample output: BranchNuX run against Demo Dashboard

This folder shows what BranchNuX **PRODUCES** when run against a real codebase (demo-dashboard, a public Next.js admin app). The source code being tested is NOT in this repo — only the outputs. To reproduce, fork demo-dashboard separately and run `branchnux init demo-dashboard-login --industry general` against it.

> **These outputs are real `branchnux` artifacts generated against the demo-dashboard project, not handcrafted samples.**  
> The HTML report (`output/login-execution-report.html`) and XLSX were produced by running `branchnux report` against a live Playwright test pass on 2026-04-27.  
...
| UAT sign-off ledger | `output/uat-sign-off.md` | Human-readable rendered version of the sign-off ledger produced by `branchnux sign --verify` |
...
```bash
git clone https://github.com/branchnux-oss/demo-dashboard.git
cd demo-dashboard
npm install
npm run build
...
Keep the server running. Open a second terminal for BranchNuX commands.

### Init a new test pass

```bash
# From the branchnux directory:
branchnux init demo-dashboard-login --industry general --target-url http://localhost:3737/login
```
...
```bash
branchnux report demo-dashboard-login
```
...
- **`PLAN ONLY` mode** — run `branchnux report demo-dashboard-login --plan-only` before execution to give stakeholders a pre-review XLSX

---

Want to use BranchNuX on YOUR codebase? → see [docs/getting-started.md](../../docs/getting-started.md)
