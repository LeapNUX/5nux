 ...
 *
 * Implements `trunknux codify <slug>`.
 *
 ...
 *
 * Usage:
 *   trunknux codify <slug> [--folder <path>] [--model <model>]
...
export async function runCodify(slug, opts = {}) {
  ...
  if (!resolvedFolder || !fs.existsSync(resolvedFolder)) {
    printError(json,
      ...
      '  Suggested workflow:\n' +
      `    1. trunknux discover <url>    # generate scenarios\n` +
      `    2. trunknux plan ${slug}      # generate test-plan.md\n` +
      `    3. trunknux codify ${slug}    # generate spec.ts (this command)\n\n` +
  ...
  if (!fs.existsSync(templatePath)) {
    printError(json,
      ...
      '  Make sure you are running trunknux from the project root and\n' +
    ...
    try {
      ...
      if (importErr.code === 'ERR_MODULE_NOT_FOUND' || importErr.code === 'MODULE_NOT_FOUND') {
        printError(json,
          ...
          `  Then re-run: trunknux codify ${slug}`,
        );
  ...
  if (!json) {
    console.log('');
    console.log('  trunknux codify — v0.2 ALPHA');
    console.log('  ─────────────────────────────────────────────────────────');
  ...
  if (json) {
    ...
    console.log(`    4. Then: trunknux report ${slug}`);
    console.log('');
  }
}
...
1. File header (REQUIRED — use exactly this):
   // Copyright (c) 2026 TrunkNuX Contributors
   // SPDX-License-Identifier: Apache-2.0
   // [VERIFY] Generated by trunknux codify v0.2 — review all selectors and assertions
...
function handleApiError(err, json, slug) {
  ...
  if (status === 429) {
    ...
    printError(json,
      ...
      '  Options:\n' +
      `    - Wait and re-run: trunknux codify ${slug}\n` +
